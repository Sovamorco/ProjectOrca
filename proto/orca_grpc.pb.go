...
func (c *orcaClient) Subscribe(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (Orca_SubscribeClient, error) {
	stream, err := c.cc.NewStream(ctx, &Orca_ServiceDesc.Streams[0], "/orca.Orca/Subscribe", opts...)
	if err != nil {
		return nil, err
	}
	x := &orcaSubscribeClient{stream}
...
type Orca_SubscribeClient interface {
	Recv() (*QueueChangeNotification, error)
	grpc.ClientStream
}

type orcaSubscribeClient struct {
	grpc.ClientStream
}

func (x *orcaSubscribeClient) Recv() (*QueueChangeNotification, error) {
	m := new(QueueChangeNotification)
...
func _Orca_Subscribe_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(emptypb.Empty)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(OrcaServer).Subscribe(m, &orcaSubscribeServer{stream})
}

type Orca_SubscribeServer interface {
	Send(*QueueChangeNotification) error
	grpc.ServerStream
}

type orcaSubscribeServer struct {
	grpc.ServerStream
}

func (x *orcaSubscribeServer) Send(m *QueueChangeNotification) error {
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _Orca_Subscribe_Handler,
			ServerStreams: true,
		},
	},
