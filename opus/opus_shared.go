...
func (e *Encoder) EncodeFloat32(pcm []float32, frameSize, maxDataBytes int) ([]byte, error) {
	pcmPtr := (*C.float)(unsafe.Pointer(&pcm[0]))

	data := make([]byte, maxDataBytes)
	dataPtr := (*C.uchar)(unsafe.Pointer(&data[0]))

	encodedC := C.opus_encode_float(e.cEncoder, pcmPtr, C.int(frameSize), dataPtr, C.opus_int32(len(data)))
	encoded := int(encodedC)
